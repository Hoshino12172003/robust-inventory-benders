...
- Solver-free Gamma tests: 35 passed, including a complete synthetic 60-run execution, byte-stable exact CSV plan projection, second-pass resume with zero repeated instance/baseline/frontier/post calls, baseline-checkpoint interruption recovery, Gamma-specific baseline solver settings, pre-solver corrupt-output rejection, and opaque filename/ZIP-member seed detection. Static audit: 14/14 passed. PR #51/#52 regression results are rerun in the final delivery check.
